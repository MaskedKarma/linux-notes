...
def km_to_miles(km):          # Kilometres to Miles conversion
    final_miles = km * 0.621371
    return final_miles

def miles_to_km(miles):         # Miles to Kilometres conversion
    final_km = miles * 1.60934
    return final_km

def kg_to_lbs(kg):            # Kilograms to Pounds conversion
    final_lbs = kg * 2.20462
    return final_lbs

def lbs_to_kg(lbs):           # Pounds to Kilograms conversion
    final_kg = lbs * 0.453592
    return final_kg

def celcius_to_fahrenheit(c):          # Celcius to Fahrenheit conversion
    final_f = (c * 9/5) + 32
    return final_f

def fahrenheit_to_celcius(f):         # Fahrenheit to Celcius
    final_c = (f - 32) * 5/9
    return final_c 
